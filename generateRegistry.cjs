...
export interface PlantIconConfig {
  ...
  bgColor: string;
  outlineColor: string;
  shapeGroup: 'mango' | 'banana' | 'round' | 'oval' | 'berry' | 'cluster' | 'palm' | 'spiky' | 'star' | 'melon' | 'citrus' | 'nut' | 'fig' | 'leaf' | 'bean' | 'flower';
}
...
fs.writeFileSync('src/components/plantIconRegistry.tsx', content);
