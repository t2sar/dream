    ...
    <div className="bg-surface-alt/5 p-4 rounded-3xl border border-surface-alt mb-6 flex flex-col gap-4">
      <div className="flex items-center justify-between">
         ...
         <div className="flex gap-2">
           <button 
             onClick={onPreview}
             className="px-4 py-2 bg-surface-alt/50 text-primary-text border border-surface-alt rounded-xl font-bold font-mono text-[10px] uppercase tracking-widest hover:bg-surface-alt transition-colors"
           >
              Preview
           </button>
           <button 
      ...
      <AnimatedModal isOpen={showManageModal} onClose={() => setShowManageModal(false)} alignment="bottom" className="!p-0 !max-w-md mx-auto !rounded-t-[32px] !rounded-b-none overflow-hidden bg-surface-card border border-surface-alt h-[85vh] flex flex-col">
         <div className="p-6 border-b border-surface-alt bg-background-main shrink-0 sticky top-0 z-10 hidden-scrollbar max-h-full overflow-y-auto w-full">
            <div className="flex justify-between items-center mb-6">
               ...
               <div className="flex gap-2 items-center">
                 <button onClick={handleExport} className="p-2 text-muted-text hover:text-primary-text transition-colors" title="Export JSON">
                   <Download className="w-4 h-4" />
                 </button>
                 <button onClick={() => fileInputRef.current?.click()} className="p-2 text-muted-text hover:text-primary-text transition-colors" title="Import JSON">
                   <Upload className="w-4 h-4" />
                 </button>
                 <input type="file" accept=".json" ref={fileInputRef} onChange={handleImport} className="hidden" />
                 <button onClick={() => setShowManageModal(false)} className="p-2 -mr-2 text-muted-text hover:text-primary-text transition-colors ml-2">
                   <X className="w-5 h-5" />
                 </button>
               </div>
            </div>
            ...
            <form onSubmit={handleSaveQuote} className="flex flex-col gap-3">
               <div className="relative">
                 <textarea 
                    ...
                    className="w-full bg-surface-soft border border-surface-alt rounded-xl p-3 text-primary-text font-sans text-sm outline-none focus:border-[#00F5D4] transition-colors min-h-[80px] resize-none pb-8"
                 />
               ...
               <div className="flex gap-2">
                 <input 
                    ...
                    className="flex-1 bg-surface-soft border border-surface-alt rounded-xl px-3 py-2 text-primary-text font-mono text-xs outline-none focus:border-[#00F5D4] transition-colors"
                 />
                 {editingId ? (
                   <div className="flex gap-2">
                      <button type="button" onClick={cancelEdit} className="bg-surface-alt text-primary-text px-4 py-2 rounded-xl font-bold font-mono tracking-widest text-[10px] uppercase hover:bg-zinc-700 transition-colors">
                        Cancel
                      </button>
                 ...
                 {motivations.map(m => (
                    <div key={m.id} className="p-4 bg-surface-soft border border-surface-alt rounded-xl group relative pr-12 transition-colors hover:border-surface-alt/80">
                       ...
                       <div className="absolute right-2 top-2 bottom-2 flex flex-col justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(m)} className="p-1.5 bg-surface-card border border-surface-alt rounded-lg text-slate-400 hover:text-primary-text hover:border-[#00F5D4] transition-all">
