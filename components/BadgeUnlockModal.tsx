...
export const BadgeUnlockModal: React.FC<BadgeUnlockModalProps> = ({ badges, onClose }) => {
  ...
  return (
    <AnimatedModal isOpen={true} onClose={onClose} className="!max-w-sm !p-0 bg-surface-card border-0 rounded-large-card w-full max-w-sm overflow-visible md:overflow-visible shadow-xl mt-16 pb-4">
        ...
        {/* Content */}
        <div className="pt-16 pb-6 px-8 flex flex-col items-center text-center relative">
          <div className="absolute -top-12 left-1/2 -translate-x-1/2">
             <div className="w-24 h-24 rounded-full bg-accent-mustard flex items-center justify-center border-[6px] border-surface-card shadow-md relative z-10 animate-companion-float">
          ...
          <button
            onClick={handleNext}
            className="w-full mt-2 py-4 bg-primary-anchor hover:opacity-90 text-surface-soft font-bold text-sm tracking-wide transition-all rounded-button shadow-sm"
          >
