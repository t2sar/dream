...
export const LivingMonolith: React.FC<LivingMonolithProps> = ({ habits, completedHabitIds, level }) => {
  ...
  const completedCount = habits.filter(h => completedHabitIds.includes(h.id)).length;
  ...
  useEffect(() => {
    if (totalHabits > 0 && completedCount === totalHabits && prevCompletedCountRef.current < totalHabits) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const shards: Shard[] = faceIndices.map((indices, faceIndex) => {
        ...
        const isThisCompleted = mappedHabit ? completedHabitIds.includes(mappedHabit.id) : false;
      ...
      const renderedFaces = shards.map((shard, index) => {
        ...
        const bounceOffset = Math.sin(offsets.phase) * (8 * (1.0 - completionRatio));
        ...
        const projectedVertices = shard.vertices.map(vertex => {
          ...
          const faceCentroid = {
            x: (shard.vertices[0].x + shard.vertices[1].x + shard.vertices[2].x) / 3,
            y: (shard.vertices[0].y + shard.vertices[1].y + shard.vertices[2].y) / 3,
            z: (shard.vertices[0].z + shard.vertices[1].z + shard.vertices[2].z) / 3,
          };

          // Apply scatter dispersion offset outward
          x3d += faceCentroid.x * (scatterDistMultiplier - 1);
          y3d += faceCentroid.y * (scatterDistMultiplier - 1) + (bounceOffset / sizeScale);
        ...
        if (mouseRef.current.active && !isFullComplete) {
          ...
          if (distance < maxRepulseDist) {
            ...
            const force = (1.0 - distance / maxRepulseDist) * 35 * (1.0 - completionRatio);
        ...
        const averageZ = (projectedVertices[0].z + projectedVertices[1].z + projectedVertices[2].z) / 3;
      ...
      renderedFaces.forEach(({ shard, coords, faceIndex }) => {
        const isMappedAndCompleted = shard.habitId ? completedHabitIds.includes(shard.habitId) : false;
      ...
      // TL Corner
      ctx.beginPath();
      ctx.moveTo(margin, margin + rSize); ctx.lineTo(margin, margin); ctx.lineTo(margin + rSize, margin);
      ctx.stroke();
      // TR Corner
      ctx.beginPath();
      ctx.moveTo(width - margin, margin + rSize); ctx.lineTo(width - margin, margin); ctx.lineTo(width - margin - rSize, margin);
      ctx.stroke();
      // BL Corner
      ctx.beginPath();
      ctx.moveTo(margin, height - margin - rSize); ctx.lineTo(margin, height - margin); ctx.lineTo(margin + rSize, height - margin);
      ctx.stroke();
      // BR Corner
      ctx.beginPath();
      ctx.moveTo(width - margin, height - margin - rSize); ctx.lineTo(width - margin, height - margin); ctx.lineTo(width - margin - rSize, height - margin);
      ctx.stroke();
  ...
  }, [habits, completedHabitIds, completionRatio, isFullComplete, isSnapping, shockwaveRadius]);
